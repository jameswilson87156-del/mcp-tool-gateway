...
项目借用了 MCP 的 Tool、Prompt、Resource 三类能力抽象和工具接入思想。P7B 增加了一个 HTTP-only JSON-RPC 2.0 envelope adapter，但只支持 `tools/list`、`tools/call`、`prompts/list`、`resources/list` 四个 demo methods；其余 API 仍是 Spring MVC REST。

它仍未实现完整 capability negotiation、stdio/SSE transport、生命周期、官方 schema interoperability。因此称为 MCP-style adapter demo 更准确，不能包装成官方 MCP server 或完整协议兼容。
...
我会分阶段替换 demo 边界：先建立真实身份与 tenant boundary，用 JWT/OAuth/SSO 替换 `X-Demo-Role`；迁移 PostgreSQL/MySQL 并加入 Flyway/Liquibase、约束、索引和数据库级分页；再用真实 MCP compatibility layer、标准 transport 和 interoperability tests 替换有限的 HTTP JSON-RPC adapter，并增加隔离的 external tool adapter。
